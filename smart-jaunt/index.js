...
module.exports = function SmartJaunt(mod) {
	const _m = {
		h: [],
		b: [],
		x: [],
	...
	try {
		...
		_m.c = {
			...
			debug: false,
			range: 35,
			range_description: 'Limit front/back teleport to monsters within x meters, measured from the center of the hitbox and will differ slightly from the value you see when you mouseover a monster.',
			angle: 0.261799,
			angle_description: 'Angle offset to auto acquire the front/back of the monster measured in radians. If no target in within this angle, you will jaunt forwards/backwards instead.',
			distance: 200,
			distance_description: 'Distance to displace you from the monster (some guess work here because I\'m unwilling to account for the hitboxes of every monster in the game). This is an offset for the game\'s { x, y, z }, not to be confused with meters.',
			direction: -1,
			direction_description: 'Jaunt forwards(1) or backwards(-1) if no target is within either 1] range or 2] angle.',
			cooldown: 3500,
			cooldown_description: 'The amount of time in ms to simulate cooldown on jaunt, during this time using jaunt again will take you back to where you were.'
		};
	...
	mod.hook('S_LOGIN', 12, ({ templateId }) => {
		_m.m = [1, 10].includes(_m.i = (templateId - 10101) % 100) ? 1 : [6, 7].includes(_m.i) ? 0 : -1;	//? 0 = Warrior, 1 = Lancer, 2 = Slayer, 3 = Berserker, 4 = Sorcerer, 5 = Archer, 6 = Priest, 7 = Mystic, 8 = Reaper, 9 = Gunner, 10 = Brawler, 11 = Ninja, 12 = Valkyrie
...
//		});
		...
		hook('C_START_INSTANCE_SKILL', 5, { order: -100 }, ({ skill, loc, w, targets }) => {
			...
			mod.send('S_ABNORMALITY_BEGIN', 3, { target: mod.game.me.gameId, source: mod.game.me.gameId, id: 2050, duration: 0, unk: 0, stacks: 1, unk2: 0, unk3: 0 });
			...
			setTimeout(() => {
				mod.send('S_ABNORMALITY_END', 1, { target: mod.game.me.gameId, id: 2050 });
				mod.send('S_ABNORMALITY_END', 1, { target: mod.game.me.gameId, id: 2060 });
